...
pool.query(
    `INSERT INTO UserMilitus2021 
                 (Fields, Data_Type, Sample_Data)
                 VALUES 
                 ('Business Name',	'Text', 'Militus'),
                 ('First Name',	'Text', 'Militus'),  
                 ('Last Name',	'Text', 'Innocent'),
                 ('Email Address', 'email',	'imilitus@gmail.com'),
                 ('Amount',	'Number',	'500000'),
                 ('Password', 'password',	'fjhglslkalmf1213/')
                 `,
    (err, res) => {
      if(err) {
        console.log(err);
